...
function LoadingScreen({ isLoading, onComplete }) {
      ...
      interval = setInterval(() => {
        setProgress((prev) => {
          if (prev >= 100) {
            ...
            setTimeout(() => setStartZoom(true), 400);
  ...
  return (
    <AnimatePresence>
      {isLoading && (
        <motion.div className="fixed inset-0 z-[9999] bg-[#635f5f] flex items-center justify-center overflow-hidden">
          <motion.div
            initial={{
              y: 100,
              opacity: 0,
            ...
            animate={{
              y: 0,
              opacity: 1,
              width: startZoom ? "100vw" : "80vw",
              height: startZoom ? "100vh" : "80vh",
              borderRadius: startZoom ? "0px" : "10px",
              scale: 1,
            }}
            transition={{
              y: { duration: 1, ease: [0.76, 0, 0.24, 1] },
              opacity: { duration: 1, ease: [0.42, 0, 0.58, 1] },
              width: { duration: 1, ease: [0.76, 0, 0.24, 1] },
              height: { duration: 1, ease: [0.76, 0, 0.24, 1] },
              borderRadius: { duration: 1, ease: [0.76, 0, 0.24, 1] },
            }}
            className="relative bg-zinc-900 text-white shadow-xl px-8 py-4 flex flex-col justify-start overflow-hidden"
          ...
          >
            ...
            <div className="mt-[6.5vw] lg:mt-[3vw] px-2">
              <TextStructure fadeInComplete={false} />
            </div>
            ...
            <div className="flex justify-center mt-10">
              <motion.div
                ...
                className="text-center text-xl text-white font-['Founders_Grotesk_XCond-Bold']"
              >
                A Clone Project developed by Khan Arkam
              </motion.div>
            </div>
            ...
            <motion.div
              animate={{ opacity: hideLoaderText ? 0 : 1 }}
              transition={{ duration: 0.5 }}
              className="absolute bottom-4 left-5 text-[14px] ml-2.5 mb-3.5 text-white/70 font-light font-['Neue_Montreal']"
            >
              Loading:
            </motion.div>
            ...
            <motion.div
              animate={{ opacity: hideLoaderText ? 0 : 1 }}
              transition={{ duration: 0.5 }}
              className="absolute bottom-4 right-5 text-7xl font-['Founders_Grotesk_XCond-Bold'] font-semibold text-white"
